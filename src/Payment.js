...
function Payment() {
  ...
  useEffect(() => {
    ...
    const getClientSecret = async () => {
      try {
        // const response = await axios.post(`/payments/create?total=${getBasketTotal(basket)}`);
        ...
        <div className="payment__section">
          ...
          <div className="payment__items">
            {basket.map((item, index) => (
              <CheckoutProduct
                key={item.id + index} // Add a unique key prop
                title={item.title}
                image={item.image}
                price={item.price}
                rating={item.rating}
...
//   const [succeeded, setSucceeded] = useState(false);
//   const [processing, setProcessing] = useState(""); // Update initial state
//   const [error, setError] = useState(null);
//   const [disabled, setDisabled] = useState(true);
